...
spaceleft = min( min(pos(:,1) - ti(:,1)), min(ext(:,1)) );
spacedown = min(min(pos(:,2) - ti(:,2)), min(ext(:,2)));
...
spaceright= figW - max(max(pos(:,1) + pos(:,3) + ti(:,3)), max(sum(ext(:,[1 3]),2)));
spaceup = figH - max(max(pos(:,2) + pos(:,4) + ti(:,4)), max(sum(ext(:,[2 4]),2)));

%moveleft = (spaceleft - spaceright) / 2;
%movedown = (spacedown - spaceup) / 2;
...
for i = 1:numel(hax)
    ...
    % Find hax that are on same row.
    sameRow = [];
    sameCol = [];
    for n = 1:numel(hax)
        if n ~= i
            if strcmpi(hax(n).Tag,'legend')
                %if ~contains(hax(n).Location,'outside')
                    continue;
                %end
            end
            ...
            [~,I2] = max([haxW, haxnW]);
            if I1 == 1
                if histcounts([pos(n,2),pos(n,2)+pos(n,4)],[pos(i,2),pos(i,2)+pos(i,4)])
                    sameRow(end+1) = n;
                end
            else
                if histcounts([pos(i,2),pos(i,2)+pos(i,4)],[pos(n,2),pos(n,2)+pos(n,4)])
                    sameRow(end+1) = n;
                end
            end
            
            if I2 == 1
                if histcounts([pos(n,1),pos(n,1)+pos(n,3)],[pos(i,1),pos(i,1)+pos(i,3)])
                    sameCol(end+1) = n;
                end
            else
                if histcounts([pos(i,1),pos(i,1)+pos(i,3)],[pos(n,1),pos(n,1)+pos(n,3)])
                    sameCol(end+1) = n;
                end
            end
        end
    end
    nRow = (length(sameRow)+1);
    nCol = (length(sameCol)+1);
    
    extraW = (spaceleft + spaceright)/nRow;
    extraH = (spaceup + spacedown)/nCol;
    
    % find position in row
    I = find( sort(pos([i,sameRow],1))  == pos(i,1));
    moveleft = spaceleft - extraW * (I-1);
    
    % find position in col
    I = find( sort(pos([i,sameCol],2))  == pos(i,2));
    movedown = spacedown - extraH * (I-1);
    
    set(hax(i), 'Position', [pos(i,1:2) - [moveleft,movedown], pos(i,3:4) + [extraW, extraH]]);
